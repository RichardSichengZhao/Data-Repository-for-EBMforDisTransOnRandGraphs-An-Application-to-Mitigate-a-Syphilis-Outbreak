...
Test500_Init27_3QT <-read.csv(paste0(wd,"/3QT_500_Init27_combined.csv"), header=T)
#Test100_Init14_3QT <-read.csv(paste0(wd,"/3QT_HighRes100_Init14_combined.csv"), header=T)

Test500_Init27_3QT_MASIR <-read.csv(paste0(wd,"/3QT_500_Init27_MASIR_combined.csv"), header=T)
Test500_Init117_3QT_MASIR <-read.csv(paste0(wd,"/3QT_500_Init117_MASIR_combined.csv"), header=T)

#Test100_Init14_3QT_MASIR <-read.csv(paste0(wd,"/3QT_HighRes100_Init14_MASIR_combined.csv"), header=T)

#readmat<-Test25_Init10
readmat <- Test500_Init27_3QT
#readmat
length(readmat[,1])
readmat[1,]
...
#readmat<-Test25_Init10
#readmat_MASIR <- Test500_Init27_3QT_MASIR
readmat_MASIR <- Test500_Init117_3QT_MASIR
#readmat
...
ggplot(df_MASIR, aes(x = beta_value_MASIR, y = p_value_MASIR, fill = log_diff_MASIR))+
  geom_tile() +
  scale_fill_gradient2(low = "white",mid="orange", midpoint = 1, high = "red")+
  labs(fill="Adjusted Loglik",x=""~beta, y="p")
...
# Checking Error
I0 <- 117
MA_Opt<- MASIR_Proc(MAOptbeta, MAGamma_fit, init_S = (N-I0)/N, ODEmaxTime=500, ODEstep=1e-1,TrackDyn = T)
MA_Opt$R0
MA_Opt$RInfinity
MA_Opt$RInfinity*N
...
# Beta
HRn <-500
MSV_Beta_CI_seq <- seq(min(readmat$beta_vec),max(readmat$beta_vec),length.out=HRn)
...
# P
MSV_P_CI_seq <- seq(min(readmat$p_vec),max(readmat$p_vec),length.out=HRn)
...
MSV_Bseq <- MSV_Beta_CI_seq
MSV_Pseq <- MSV_P_CI_seq
...
MSV_CIsub <- subset(MSV_CI,MSV_CI$MSVWald_Value_CI<MSV_Stat & MSV_CI$lik_value > -Inf)
...
ggplot(MSV_CI, aes(x = beta_value, y = p_value, fill = log_diff))+
  ...
  labs(fill="Adjusted Loglik",x=""~beta~"(bi-week\u207b\u00b9)", y="p")+
  new_scale_fill()+
  geom_tile(data = MSV_CIsub,aes(x = beta_value, y = p_value,fill="95% Wald CI area"),alpha=0.20,inherit.aes = FALSE)+
  scale_fill_manual(values = c("blue"))+
  labs(fill="", linetype="")+
  annotate("errorbar",x=Optbeta,y=Optp,xmax=MSV_Beta_UB,xmin=MSV_Beta_LB,width=0.01,linewidth=0.05)+
  annotate("errorbar",x=Optbeta,ymax=MSV_P_UB,ymin=MSV_P_LB,width=0.00008,linewidth=0.05)
#+xlim(min(beta_value),0.007)
...
I0
# beta derivative
MASIR_Opt_plusBeta <- MASIR_Proc(MAOptbeta+d/2,MAGamma_fit,init_S = (N-I0)/N, ODEmaxTime = fitend_tps, ODEstep = 1e-1,TrackDyn = T)
MASIR_R_Opt_plusBeta <- MASIR_Opt_plusBeta$Dynamic[fitend_timeseq,4]*N
MASIR_Opt_minusBeta <- MASIR_Proc(MAOptbeta-d/2,MAGamma_fit,init_S = (N-I0)/N, ODEmaxTime = fitend_tps, ODEstep = 1e-1,TrackDyn = T)
...
MASIR_Opt_plusP <- MASIR_Proc(MAOptbeta,MASIR_Gamma_vec[3],init_S = (N-I0)/N, ODEmaxTime = fitend_tps, ODEstep = 1e-1,TrackDyn = T)
MASIR_R_Opt_plusP <- MASIR_Opt_plusP$Dynamic[fitend_timeseq,4]*N
MASIR_Opt_minusP <- MASIR_Proc(MAOptbeta,MASIR_Gamma_vec[1],init_S = (N-I0)/N, ODEmaxTime = fitend_tps, ODEstep = 1e-1,TrackDyn = T)
...
# Beta
HRn <-500
MASIR_Beta_CI_seq <- seq(min(readmat_MASIR$beta_vec),max(readmat_MASIR$beta_vec),length.out=HRn)
...
# P
MASIR_P_CI_seq <- seq(min(readmat_MASIR$p_vec),max(readmat_MASIR$p_vec),length.out=HRn)
...
MASIR_Bseq <- MASIR_Beta_CI_seq
MASIR_Pseq <- MASIR_P_CI_seq
...
for (i in c(1:length(df_MASIR$beta_value))) {
  vec <- matrix(c(MAOptbeta-df_MASIR[i,1],MAOptp-df_MASIR[i,3]),nrow = 1,ncol = 2)
  MASIRWald_Value_CI[i] <- vec %*% MASIR_GTGmat %*% t(vec)
}

MASIR_CI <- cbind(df_MASIR,MASIRWald_Value_CI)
MASIR_CIsub <- subset(MASIR_CI,MASIR_CI$MASIRWald_Value_CI<MASIR_Stat & MASIR_CI$lik_value_MASIR>-Inf)
...
ggplot(MASIR_CI, aes(x = beta_value_MASIR, y = p_value_MASIR, fill = log_diff_MASIR))+
  ...
  labs(fill="Adjusted Loglik",x=""~beta~"(bi-week\u207b\u00b9)", y="p")+
  new_scale_fill()+
  geom_tile(data = MASIR_CIsub,aes(x = beta_value_MASIR, y = p_value_MASIR,fill="95% Wald CI area"),alpha=0.20,inherit.aes = FALSE)+
  scale_fill_manual(values = c("blue"))+
  labs(fill="")+
  annotate("errorbar",x=MAOptbeta,y=MAOptp,xmax=MASIR_Beta_UB,xmin=MASIR_Beta_LB,width=0.01,linewidth=0.05)+
  annotate("errorbar",x=MAOptbeta,y=MAOptp,ymax=MASIR_P_UB,ymin=MASIR_P_LB,width=0.0010,linewidth=0.05)
