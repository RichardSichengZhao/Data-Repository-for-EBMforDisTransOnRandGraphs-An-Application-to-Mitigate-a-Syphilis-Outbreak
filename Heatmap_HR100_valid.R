...
Test100_Init27_Valid <- read.csv(paste0(wd,"/Valid_HighRes100_Init27_combined.csv"), header=T)
...
readmat_full <- Test100_Init27_3QT
readmat_split <- Test100_Init27_Valid
#readmat
length(readmat_full[,1])
length(readmat_split[,1])
#readmat[1,]

beta_value <- readmat_full[,4]
gamma_value <- readmat_full[,3]
p_value <- readmat_full[,2]
lik_value <- readmat_full[,5]
log_diff <- -log(max(lik_value)-lik_value+0.001)

df_full <- data.frame(beta_value,gamma_value,p_value,lik_value,log_diff)

beta_value <- readmat_split[,4]
gamma_value <- readmat_split[,3]
p_value <- readmat_split[,2]
lik_value <- readmat_split[,5]
log_diff <- -log(max(lik_value)-lik_value+0.001)

df_split <- data.frame(beta_value,gamma_value,p_value,lik_value,log_diff)


#dfs <- subset(df,df$beta_value<0.15&df$beta_value>0.05)

Opt_full <- df_full[which.max(df_full$lik_value),]
Opt_split <- df_split[which.max(df_split$lik_value),]
...
Beta_full <- Opt_full[1,1]
Beta_split <- Opt_split[1,1]
# beta = 0.0037 bi-weekly
Gamma_full <- Opt_full[1,2]
Gamma_split <- Opt_split[1,2]

# gamma=0.005329582 bi-weekly
p_full <- Opt_full[1,3]
p_split <- Opt_split[1,3]
# p = 0.14

# Checking Error
CM_full<- ModProc_CM(DDist,Beta_full,Gamma_full,ODEmaxTime = 500, ODEstep = 1e-1,init_theta = it_theta,TrackDyn = T)
CM_full$R0
CM_full$RInfinity
CM_full$RInfinity*N

CM_split<- ModProc_CM(DDist,Beta_split,Gamma_split,ODEmaxTime = 500, ODEstep = 1e-1,init_theta = it_theta,TrackDyn = T)
CM_split$R0
CM_split$RInfinity
CM_split$RInfinity*N

Sys_full <- CM_full$Dynamic
full_times_fit <- Sys_full[fitend_timeseq,]
full_times <- Sys_full[timeseq,]

Sys_split <- CM_split$Dynamic
split_times_fit <- Sys_split[fitend_timeseq,]
split_times <- Sys_split[timeseq,]

Rcount_full <- full_times_fit[,3]*N
Rcount_split <- split_times_fit[,3]*N

full_times_fit <- cbind(full_times_fit,Rcount_full)
split_times_fit <- cbind(split_times_fit,Rcount_split)

Rcount_full_all <- full_times[,3]*N
full_times <- cbind(full_times,Rcount_full_all)

Rcount_split_all <- split_times[,3]*N
split_times <- cbind(split_times,Rcount_split_all)


inc_count_full_all <- rep(0,tps)
inc_count_full <- rep(0,fitend_tps)

inc_count_split_all <- rep(0,tps)
inc_count_split <- rep(0,fitend_tps)

for (i in c(1:fitend_tps)){
  inc_count_full[i] <- full_times_fit[i+1,6]-full_times_fit[i,6]
  inc_count_split[i]<- split_times_fit[i+1,6]-split_times_fit[i,6]
}
for (i in c(1:tps)){
  inc_count_full_all[i] <- full_times[i+1,6]-full_times[i,6]
  inc_count_split_all[i] <- split_times[i+1,6]-split_times[i,6]
}

# Incident Compare
inc_exp_full <- inc_count_full_all*p_full
inc_exp_split <- inc_count_split_all*p_split
...
Valid_data_fit <- read.csv(paste0(wd,"/Valid_data_fit.csv"))
Valid_data_test <- read.csv(paste0(wd,"/Valid_data_test.csv"))

t_data <- seq(1,tps)
df_full_inc <- as.data.frame(cbind(t_data,inc_count_full_all,inc_exp_full))
df_split_inc <- as.data.frame(cbind(t_data,inc_count_split_all,inc_exp_split))


full_test <- df_full_inc[Valid_data_test$tps_test,]$inc_count_full_all
split_test <- df_split_inc[Valid_data_test$tps_test,]$inc_count_split_all

test_mat<-cbind(Valid_data_test,full_test,split_test)
...
test_length <- length(test_mat$tps_test)
LogLik_vec_full <- rep(0,test_length)
LogLik_vec_split <- rep(0,test_length)

for (k in c(1:test_length)){
  obs <- Valid_data_fit[k,2]
  full_pred <- test_mat$full_test[k]
  split_pred <- test_mat$split_test[k]
  LogLik_vec_full[k] <- log(Den_cb_est(full_pred,obs,p_full))
  LogLik_vec_split[k] <- log(Den_cb_est(split_pred,obs,p_split))
}
LogLik_full <-sum(LogLik_vec_full)
LogLik_split <-sum(LogLik_vec_split)
...
df_datafit <- as.data.frame(Valid_data_fit)
df_datatest <- as.data.frame(Valid_data_test)
...
#   scale_color_manual(breaks=c("Network Model","Fitting data","Unfitted data"),values=c("Network Model"="red","Fitting data"="blue","Unfitted data"="green"))+
#   labs(x="Time(bi-week)",y="Casecount",color="Color",linetype="Line Type")
inc_exp_full
ggplot()+theme_classic()+
  geom_line(data = df_full_inc,aes(x=t_data, y=inc_exp_full,color="Full model"))+
  geom_line(data = df_split_inc,aes(x=t_data, y=inc_exp_split,color="Split model"))+
  geom_vline(aes(xintercept=fitend_tps),linewidth=0.8,linetype="dotted")+
  geom_point(data=df_datafit,aes(x=tps_fit,y=fit_data,color="Split: Fitting data (n=87)"), alpha=0.5)+
  geom_point(data=df_datatest,aes(x=tps_test,y=test_data,color="Split: Testing data (n=39)"), alpha=0.5)+
  geom_point(data=df_notfit,aes(x=t_data,y=R_data,color="Unused data (n=16)"), alpha=0.5)+
  #geom_line(data=df_data,aes(x=t_data,y=rsum,color="Runing average of data (RN=5)"), alpha=0.5)+
  #scale_linetype_manual(values=c("Expectation"="dashed","Prediction"="solid"))+
  scale_color_manual( breaks=c(  "Full model"
                               , "Split model"
                               , "Split: Fitting data (n=87)"
                               , "Split: Testing data (n=39)"
                               , "Unused data (n=16)"
                               #, "Runing average of data (RN=5)"
                               )
                    , values=c( "Full model"="red"
                              , "Split model"="blue"
                              , "Split: Fitting data (n=87)"="blue"
                              , "Split: Testing data (n=39)"="brown"
                              , "Unused data (n=16)"="green"
                              # ,"Runing average of data (RN=5)"="black"
                              )
                    )+
  labs(x="Time (bi-week)",y="Casecount",color="Color",linetype="Line Type")
