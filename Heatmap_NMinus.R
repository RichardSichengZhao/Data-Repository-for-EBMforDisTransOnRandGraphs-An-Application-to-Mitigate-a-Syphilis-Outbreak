...
Test100_NMinus_3QT <-read.csv(paste0(wd,"/3QT_Test100_NMinus_combined.csv"), header=T)
#Test100_NPlus_3QT_MASIR <-read.csv(paste0(wd,"/3QT_Test100_NPlus_MASIR_combined.csv"), header=T)

#readmat<-Test25_Init10
readmat <- Test100_NMinus_3QT
...
ggplot(df_Init, aes(x=Init_seq,y=MaxLik_seq))+theme_bw()+
  geom_point()+
  annotate("point",x=which.max(MaxLik_seq),y=max(MaxLik_seq),colour="red")+
  ylim(-200,-165)+
  xlim(10,50)+
...
ggplot(ListInit[[23]], aes(x = beta_value, y = p_value, fill = log_diff))+
  geom_tile() +
...
ggplot(df_Init_MASIR, aes(x=Init_seq_MASIR,y=MaxLik_seq_MASIR))+
  geom_point()+
  annotate("point",x=I0_vec_MASIR[which.max(MaxLik_seq_MASIR)],y=max(MaxLik_seq_MASIR),colour="red")+theme_bw()+
  annotate("point",x=27,y=ListMaxInd_MASIR[[27]]$lik_value_MASIR,colour="green")+
  ylim(-190,-170)+
